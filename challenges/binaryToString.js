...
function decodePass( passArr, bin ){
  ...
  // binary to string
  var binArr = bin.split(" "); // ['01110000', '01100001', ....]
  var pwd = '';
  
  binArr.forEach(function (item) {
    var int = parseInt(item, 2); // 112
    var intStr = String.fromCharCode(int); // '01110000' -> 'p'
    pwd += intStr;
  })
  
  // check if the pwd(password) is the same as item
  var result = false;
  passArr.forEach(function(item) {
    if(item === pwd) result = item;
  })
  return result;
}
